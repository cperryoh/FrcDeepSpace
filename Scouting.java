...
public class Scouting {
	...
	public Scouting() throws IOException {
		...
		frmMadeByCole.setLocation((width / 2) - (frmMadeByCole.getWidth() / 2),
				(height / 2) - (frmMadeByCole.getHeight()));
	...
	private void initialize() {
		...
		frmMadeByCole.setLocation((width / 2) - (frmMadeByCole.getWidth() / 2),
				(height / 2) - (frmMadeByCole.getHeight() / 2));
	...
	void enter() throws IOException {
		if (enterable) {
			try {
				...
				writer2.println();
				// creates table
				int[][] hatchTimes = new int[2][tableModle.getRowCount()],
						cargoTimes = new int[2][tableModle.getRowCount()];
				for (int x = 0; x < tableModle.getRowCount(); x++) {
					...
					if (getTableValue(tableModle, x, 3).startsWith("hatch")
							&& (!getTableValue(tableModle, x, 2).equals("Failed"))) {
